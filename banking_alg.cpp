...
pair<double, int> calculateDensityAndCost(const vector<Cell> &cells, const vector<vector<Bin>> &bins)
{
    ...
    for (const auto &cell : cells)
    { // cell pointers
        if (const auto *flipFlop = dynamic_cast<const FlipFlop *>(&cell))
        { // attempt to cast cell pointer to flipflop
...
{
    auto [currentCost, _] = calculateDensityAndCost(, bins); // initial cost
    srand(time(nullptr));
