...
void update_banking_clusters(vector<Point> & clusters, vector<Point> & banking_clusters)
{
    for (const auto &center : clusters)
    {
...
int main()
{
    ...
    for (auto it = clk_map.begin(); it != clk_map.end(); it++)
    {
        ...
        update_banking_clusters(clusters, banking_clusters);
        write_clusters_to_file(clusters, outfile);
    }
    outfile.close();
